...
$date2 = date('Y-m-d', strtotime('-120 days'));
if(isset($_POST['time_period']))
{
    ...
    else if($var='quarterly'){
        $date1 = date('Y-m-d', time());
        $date2 = date('Y-m-d', strtotime('-120 days'));
    }  
    else if($var='yearly'){
        $date1 = date('Y-m-d', time());
        $date2 = date('Y-m-d', strtotime('-360 days'));
    }  
}
?>
...
<head>
    ...
    <title>Transactions Quarterly</title>
</head>
        ...
        <div class="h-screen flex-grow-1 overflow-y-lg-auto">
            ...
            <main class="py-5 bg-surface-secondary">
                <div class="container-fluid pb-5">
                    ...
                    <select name="time_period" id="type" onchange="javascript:location.href = this.value;" style="margin: 5px 9px; padding: 6px 13px !important; border-radius: 5px;">
                    <option value="" selected></option>
                        <option value="./transaction_monthly.php" selected>monthly</option>
                    ...
                    <h3 class="h3 mb-0 ls-tight">quarterly data</h3>
                            ...
                            <table class="table table-hover table-nowrap" id="myTable"
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        $stmt="SELECT transactions.borrow_id ,transactions.book_id ,transactions.user_id ,transactions.borrowed_date ,transactions.returned_date ,users.name ,book_name.book_name   FROM `transactions` JOIN `users` ON (transactions.user_id=users.id) JOIN `book_name` ON (transactions.book_id=book_name.book_id) where  borrowed_date between '$date2' and '$date1'";
                                        $sql=mysqli_prepare($conn, $stmt);
                                        $result=mysqli_stmt_execute($sql);
